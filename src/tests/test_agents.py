...
model_config = {
    ...
    'model': 'qwen2.5:32b',  # 或其他你在 Ollama 中安装的模型
    'parameters': {
        'temperature': 0.7,
        'top_p': 0.6,
        'max_tokens': 1500,
...
print("使用的模型配置:", model_config)  # 添加调试信息
...
async def run_test():
    try:
        ...
        while current_round < max_rounds:
            try:
                current_round += 1
                print(f"\n=== 开始第 {current_round}/{max_rounds} 轮评估 ===\n")
                ...
                print(f"DiagnosisAgent 使用的模型: {model_config['model']}")
                print(f"DiagnosisAgent 使用的API地址: {model_config['base_url']}")
                ...
                try:
                    ...
                    files = [f for f in os.listdir(results_dir) if f.startswith('assessment_')]
                    if not files:
                        print("警告：没有找到结果文件")
                    else:
                        latest_result = max(files, key=lambda x: os.path.getctime(os.path.join(results_dir, x)))
                        result_path = os.path.join(results_dir, latest_result)
                        print(f"找到最新结果文件: {result_path}")
